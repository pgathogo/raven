...
const QString class_name = "MainWindow";
    ...
    , m_outputB{nullptr}
{
    ...
    // m_current_playing_item.item = nullptr;

    m_current_cued_item = CurrentCuedItem();
    m_current_playing_item = CurrentPlayItem();
    ...
    log_info("Going to compute schedule time.");
...
//    connect(ui->btnSegue, &QPushButton::clicked, this, [&](){ ui->swMain->setCurrentIndex(2); m_control_page = ControlPage::Segue; });
    ...
    connect(ui->btnPrintSchedule, &QPushButton::clicked, this, &MainWindow::print_schedule);
...
//    connect(ui->btnSearch, &QPushButton::clicked, this, &MainWindow::search_audio);
    ...
    connect(this, &MainWindow::output_C_stopped, m_jingle_grid.get(), &OATS::JingleGrid::stop_play);

    m_current_jingle_item = std::make_unique<OATS::ScheduleItem>();
    //test_concept();

    log_info("style page controls...");

    style_page_controls();
    style_footer_buttons();

    log_info("Calculate time statistics...");

    calculate_time_stats();

    log_info("Time stats done.");
    ...
    //QLabel* top_label = new QLabel("This is a TOP Label");
    ui->hlTop->addWidget(m_audio_meter.get());
...
void MainWindow::compute_schedule_time()
{
    ...
    int time_counter = m_current_playing_item.item->audio()->playable_duration();
    ...
    for (int i=m_current_playing_item.schedule_index+1; i < m_schedule_items.size()-1; ++i){
        ...
        time_counter = time_counter + si->audio()->playable_duration();
...
void MainWindow::fetch_schedule_bydate(QDate date)
{
    ...
    sql << " SELECT a.id, a.schedule_date, a.schedule_hour, a.schedule_time, "
        ...
        << " a.audio_id, b.title, b.filepath, b.duration, b.intro_marker, b.end_marker, b.file_extension,"
...
{
    QString fetch_data = QString("Featching data from DB Date: %1, Hour: %2")
    ...
    QString sched_str = QString("Schedule records: %1").arg(QString::number(provider->cacheSize()));
    log_info(sched_str);
    ...
    do{
        ...
        {
           // Initialize the current playing item with the first item
           // in the current hour.

            m_current_playing_item.item = sched_item.get();
            m_current_playing_item.schedule_index = sched_item->index();
            m_current_playing_item.grid_index = 0;
            hour_top_item_picked = true;
        }
        ...
        if (sched_item->schedule_type() == OATS::ScheduleType::COMM)
        {
            ...
            if (comm_duration > 0)
            {
                ...
                item_duration = item_duration + sched_item->audio()->playable_duration();

                               // (sched_item->audio()->duration()->value() -
                               //  sched_item->audio()->end_marker()->value());
...
{
    qInfo() << "Fetch commercial by date...";
    ...
    qInfo() << QString("Commercial count found: %1").arg(QString::number(provider->cacheSize()));
                                     ...
                                     OATS::ScheduleItem* schedule)
{
    ...
    for (; it_begin != it_end; ++it_begin){
        ...
        if (field_name == "schedule_time"){
            ...
            schedule->set_break_time(QTime::fromString(QString::fromStdString(field_value), "hh:mm:ss"));
        }
...
//            schedule->set_auto_transition(str_to_int(field_value));
        ...
        if (field_name == "duration"){
            audio->set_duration(str_to_double(field_value));
            //schedule->set_break_duration(str_to_int(field_value));
        }

        if (field_name == "intro_marker") {
            audio->set_intro_marker(str_to_double(field_value));
        }

        if (field_name == "end_marker") {
            audio->set_end_marker(str_to_double(field_value));
        }
...
void MainWindow::make_playlist_grid()
{
    ...
    for (int i=0; i < MAX_GRID_ITEMS; ++i)
    {
        ...
        connect(grid_item.get(), &OATS::ScheduleGridItem::fade_audio, this, &MainWindow::fade_playing_audio);
        connect(grid_item.get(), &OATS::ScheduleGridItem::pause_audio, this, &MainWindow::pause_playing_audio);
...
OATS::OutputPanel* MainWindow::create_output_panel(const QString panel_name)
{
    ...
    panel->set_id(m_output_panels.size()+1);
    ...
    connect(panel.get(), &OATS::OutputPanel::fade_audio, this, &MainWindow::fade_button);
    connect(panel.get(), &OATS::OutputPanel::pause_audio, this, &MainWindow::pause_button);
...
OATS::OutputPanel* MainWindow::create_output_panel_c()
{
    auto panel = std::make_unique<OATS::OutputPanel>("C");

    panel->set_id(m_output_panels.size());
    ui->hlOutput->addWidget(panel.get());

    connect(panel.get(), &OATS::OutputPanel::play_item, this, &MainWindow::play_c_button);
    connect(panel.get(), &OATS::OutputPanel::stop_play, this, &MainWindow::stop_c_button);
    connect(panel.get(), &OATS::OutputPanel::fade_audio, this, &MainWindow::fade_c_button);
    connect(panel.get(), &OATS::OutputPanel::pause_audio, this, &MainWindow::pause_c_button);
...
{
    if (op->panel_status() == OATS::PanelStatus::PAUSED)
    {
        std::cout << "Resume audio ....\n" ;
        resume_audio(op);
        return;
...
void MainWindow::stop_button(OATS::OutputPanel* op)
{
    if (op->panel_status() == OATS::PanelStatus::PLAYING)
    {
        ...
        QString filename = op->schedule_item()->audio()->full_audio_filename();
         ...
         QString msg = QString("Manual Stop: OP: %1  : Audio: %2 - %3 ")
                                    .arg(op->panel_name())
                                    .arg(audio_name)
                                    .arg(filename);
        log_info(msg);
...
void MainWindow::fade_button(OATS::OutputPanel* op)
{
    fade_audio(op);
}

void MainWindow::pause_button(OATS::OutputPanel* op)
{
    pause_audio(op);
}

void MainWindow::play_c_button()
{
    play_outputC(m_outputC);

}

void MainWindow::stop_c_button()
{
    if (m_outputC->panel_status() == OATS::PanelStatus::PLAYING)
    {
        m_outputC->schedule_item()->set_transition_type(OATS::TransitionType::STOP);
        m_jingle_player->stop_play();


        QString audio_name = m_outputC->schedule_item()->audio()->title()->to_qstring();
        QString filename = m_outputC->schedule_item()->audio()->full_audio_filename();

         QString msg = QString("Manual Stop: OP: %1  : Audio: %2 - %3 ")
                                    .arg(m_outputC->panel_name())
                                    .arg(audio_name)
                                    .arg(filename);
        log_info(msg);
    }
...
void MainWindow::fade_c_button()
{
    fade_audio(m_outputC);
}


void MainWindow::pause_c_button()
{
    pause_jingle(m_outputC);
}
...
void MainWindow::display_schedule(int grid_pos, int from_item_index)
{
    ...
    for (int index=grid_pos; index <= MAX_GRID_ITEMS-1; ++index)
    {
        ...
        std::cout << "Item Index: " << from_item_index << "  CH: " << ch << '\n';
        if (prev_ch.empty()){
            prev_ch = ch;
        } else {
            if (ch == prev_ch){
...
{
    for(const auto& si : m_schedule_items) {
        std::string str_time = si->schedule_time().toString("hh:mm:ss").toStdString();
        std::cout << str_time << ": " << si->schedule_ref() << ": "<< si->audio()->title()->value() <<  '\n';
    }
}
    ...
    for(int i=from_pos; i<MAX_GRID_ITEMS-1; ++i)
    {
        if (i > 0)
        {
            ...
            if (prev_schedule_item->schedule_type() != OATS::ScheduleType::HOUR_HEADER){
                new_time = prev_schedule_time.addMSecs( prev_schedule_item->audio()->playable_duration() );
                                        // prev_schedule_item->audio()->duration()->value() -
                                        // prev_schedule_item->audio()->end_marker()->value());
            }else{
...
QTime MainWindow::schedule_time_at(int index)
{
    ...
    if (prev_schedule_item->schedule_type() != OATS::ScheduleType::HOUR_HEADER){
        new_time = prev_schedule_item->schedule_time().addMSecs( prev_schedule_item->audio()->playable_duration());
            // prev_schedule_item->audio()->duration()->value() -
            // prev_schedule_item->audio()->end_marker()->value()
            // );
    }else{
...
void MainWindow::go_to_current_hour()
{
    int current_hour = QTime::currentTime().hour();
    ...
    for (int i=0; i < m_schedule_items.size() - 1; ++i)
    {
        if (m_schedule_items[i]->schedule_type() == OATS::ScheduleType::HOUR_HEADER &&
            m_schedule_items[i]->hour() == current_hour )
        {
            header_index = i;
            break;
...
void MainWindow::play_jingle(OATS::Jingle* jingle)
{
    ...
    cue_schedule_item(m_current_jingle_item.get(), m_outputC);
...
{
    m_outputC->set_panel_status(OATS::PanelStatus::STOP);
    m_outputC->schedule_item()->set_item_status(OATS::ItemStatus::STOP);
    m_jingle_player->stop_play();
}
...
void MainWindow::set_current_play_item()
{
    ...
    //{
     ...
     m_current_playing_item.item = new OATS::ScheduleItem();
     m_current_playing_item.schedule_index = -1;
     m_current_playing_item.grid_index = -1;

     /*
     for (auto& s_item: m_schedule_items)
     {
        ...
        m_current_playing_item.item = s_item.get();
...
void MainWindow::slow_flash()
{
    ...
    if (m_outputA->panel_status() == OATS::PanelStatus::CUED &&
        m_outputA->start_trigger_tick_stamp() > 0){
    ...
    if (m_outputB->panel_status() == OATS::PanelStatus::CUED &&
        m_outputB->start_trigger_tick_stamp() > 0)
    {
...
void MainWindow::count_down()
{
    ...
    case OATS::PanelStatus::PLAYING: {
        ...
        if (m_outputA->panel_status()== OATS::PanelStatus::PAUSED)
            return;

        long long remaining = 0;
        long long elapsed = 0;
        double  audio_duration = m_outputA->schedule_item()->audio()->playable_duration();

        if (m_outputA->pause_tick_stamp() > 0 ) {
            elapsed =  m_outputA->pause_tick_stamp() - m_outputA->start_tick_stamp();
            m_outputA->set_start_tick_stamp(trigger_ticker-elapsed);
            m_outputA->set_pause_tick_stamp(0);

        } else {
            elapsed = trigger_ticker - m_outputA->start_tick_stamp();
        }

        if (m_outputA->forced_fade_out())
        {
          remaining  = m_outputA->forced_fade_duration() - OATS::FORCED_FADE_OUT_COUNT_DOWN;
          m_outputA->set_forced_fade_duration(remaining);
        ...
        if ( elapsed < m_outputA->schedule_item()->audio()->intro_marker()->value() )
        {
            ...
            */
            ...
            m_outputA->update_progress_bar(100 - round(  ((float)m_outputA->time_remaining() /
                                 (audio_duration - m_outputA->schedule_item()->audio()->intro_marker()->value()) ) * 100 ) );
        }
    ...
    switch (m_outputB->panel_status())
    {
     ...
     case OATS::PanelStatus::PLAYING:
      {
         if (m_outputB->panel_status() == OATS::PanelStatus::PAUSED)
             return;

         long long remaining = 0;
         long long elapsed = 0;
         double  audio_duration = m_outputB->schedule_item()->audio()->playable_duration();

         if (m_outputB->forced_fade_out()){

            remaining  = m_outputB->forced_fade_duration() - OATS::FORCED_FADE_OUT_COUNT_DOWN;
            m_outputB->set_forced_fade_duration(remaining);

         } else {

            if (m_outputB->pause_tick_stamp() > 0 )
            {
                elapsed =  m_outputB->pause_tick_stamp() - m_outputB->start_tick_stamp();
                m_outputB->set_start_tick_stamp(trigger_ticker-elapsed);
                m_outputB->set_pause_tick_stamp(0);
    ...
    case OATS::PanelStatus::PLAYING:
    {
        if (m_outputC->panel_status() == OATS::PanelStatus::PAUSED)
            return;

        long long remaining = 0;
        long long elapsed = 0;

        double audio_duration = m_outputC->schedule_item()->audio()->playable_duration();

        if (m_outputC->forced_fade_out()) {
            remaining = m_outputC->forced_fade_duration() - OATS::FORCED_FADE_OUT_COUNT_DOWN;
            m_outputC->set_forced_fade_duration(remaining);
        } else {

            if (m_outputC->pause_tick_stamp() > 0 )
            {
                elapsed	= m_outputC->pause_tick_stamp() - m_outputC->start_tick_stamp();
                m_outputC->set_start_tick_stamp(trigger_ticker - elapsed);
                m_outputC->set_pause_tick_stamp(0);
        ...
        if ( elapsed < m_outputC->schedule_item()->audio()->intro_marker()->value() )
        {
            ...
            m_outputC->update_progress_bar( 100 - round(( ((float)(m_outputC->schedule_item()->audio()->intro_marker()->value() - elapsed ) /
                     ...
                     !m_outputC->forced_fade_duration() )
        {
            int duration_seconds = (int)remaining / 1000;
            int ds_u1 = (int)duration_seconds / 60;
            int ds_u2 = duration_seconds % 60;
     ...
     case OATS::PanelStatus::PLAYING:
         ...
         if (m_outputA->panel_status() == OATS::PanelStatus::PAUSED)
          ...
          if (m_outputA->start_tick_stamp() > 0)
          {
              if (m_outputA->forced_fade_out()){
                  elapsed = trigger_ticker - m_outputA->start_tick_stamp();
                  remaining = m_outputA->forced_fade_stamp() - OATS::FORCED_FADE_OUT_STATUS_COUNT;
                  if (remaining < 0)
                      remaining = 0;

                  m_outputA->set_forced_fade_stamp(remaining);
    ...
    switch(m_outputB->panel_status())
    {
     ...
     case OATS::PanelStatus::PLAYING:
         ...
         if (m_outputB->start_tick_stamp() > 0)
         {
             if (m_outputB->forced_fade_out()){
                 elapsed = trigger_ticker - m_outputB->start_tick_stamp();
                 remaining = m_outputB->forced_fade_stamp() - OATS::FORCED_FADE_OUT_STATUS_COUNT;
                 if (remaining < 0)
                     remaining = 0;

                     m_outputB->set_forced_fade_stamp(remaining);
                 ...
                 remaining = m_outputB->schedule_item()->audio()->playable_duration() - elapsed;
       ...
       case OATS::PanelStatus::PLAYING:
           ...
           if (m_outputC->start_tick_stamp() > 0)
           {
               if (m_outputC->forced_fade_out()){
                   ...
                   remaining = m_outputC->forced_fade_stamp() - OATS::FORCED_FADE_OUT_STATUS_COUNT;

                   if (remaining < 0)
                       remaining = 0;

                   m_outputC->set_forced_fade_stamp(remaining);
                   ...
                   remaining = m_outputC->schedule_item()->audio()->playable_duration() - elapsed;
               }

               m_outputC->set_time_remaining(remaining);

               if ( (remaining <= 0) || (m_outputC->start_tick_stamp() < 0) ) {
                   stop_audio(m_outputC);
               }
...
OATS::ScheduleItem* MainWindow::find_next_schedule_item(OATS::ScheduleItem* current_item)
{
    ...
    for(int i=index; i < m_schedule_items.size()-1; ++i)
    {
        ...
        if (next_item->schedule_time() > current_item->schedule_time()) {
...
void MainWindow::cue_schedule_item(OATS::ScheduleItem* next_schedule_item, OATS::OutputPanel* next_output_panel)
{
    ...
    auto duration_seconds = (int)(next_schedule_item->audio()->playable_duration()/1000);
...
void MainWindow::calculate_trigger_times()
{
    ...
    {
        double audio_duration = m_current_playing_item.item->audio()->playable_duration();
        ...
        if (time_yield > 0){
            ...
            if (time_yield > 500) {
                ...
                m_current_playing_item.item->audio()->set_duration( audio_duration - time_yield );
        ...
        {
            double next_audio_duration = next_schedule_item->audio()->playable_duration();
            ...
            if (m_current_playing_item.item->transition_type() == OATS::TransitionType::STOP)
            {
                ...
                track_start_mark = (current_play_start_time + audio_duration);  // - 500;
                ...
                if (m_current_playing_item.item->audio()->fade_delay_marker()->value() >
                        ...
                        m_current_playing_item.item->audio()->set_fade_delay_marker((next_audio_duration -
                                                                     next_schedule_item->audio()->fade_delay_marker()->value()) ); // - 500);
                }

                track_start_mark = (current_play_start_time + audio_duration) -
                                   m_current_playing_item.item->audio()->fade_delay_marker()->value();

                fade_start_mark = (current_play_start_time + audio_duration)-
...
void MainWindow::calculate_time_stats()
{
    ...
    // long msec_in_an_hour = 3600000;
    // long msec_in_a_minute = 60000;
    // long msec_in_a_second = 1000

    long hour_remaining = 3600000 - ((min * 60000) + (sec * 1000) + msec);

    long time_counter = 0.0;

    if (m_current_playing_item.item == nullptr)
        return;

    // FIXME: Check why application crashes in the following line
    if (m_current_playing_item.item->play_channel() == ChannelA)
    {
        if (m_outputA->panel_status() == OATS::PanelStatus::PLAYING)
        {
            ...
            time_counter = m_current_playing_item.item->audio()->playable_duration();
        }
    }
    ...
    if (m_current_playing_item.item->schedule_type() != OATS::ScheduleType::HOUR_HEADER) {
        if (m_current_playing_item.item->transition_type() == OATS::TransitionType::MIX){
            ...
            //TODO: Initialize audio in m_current_playing_item.item
            if ( m_current_playing_item.item->audio() != nullptr) {
    ...
    for (int i=m_current_playing_item.schedule_index+1; i <= m_schedule_items.size(); ++i)
    {
        if (m_schedule_items[i]->schedule_type()== OATS::ScheduleType::HOUR_HEADER)
            break;
        time_counter += m_schedule_items[i]->audio()->playable_duration();
...
void MainWindow::play_audio(OATS::OutputPanel* op)
{
    ...
    if (op->panel_status() == OATS::PanelStatus::CUED)
    {
        ...
        if (op->schedule_item()->schedule_type() == OATS::ScheduleType::COMM)
        {
            ...
            {
                QString base_filename = QString::fromStdString(m_audio_tool.make_audio_filename(item.audio_id));
                auto audio_file = item.filepath+ base_filename+ "."+ item.file_extension;
            ...
            QString audio_filepath = audio->full_audio_filename();

            QString msg = QString("Play: %1").arg(audio_filepath);
            log_info(msg);
        ...
        {
            // Set output channel for the next item
            int next_index = index_of(next_schedule_item->schedule_ref());
            if (op->schedule_item()->play_channel() == ChannelA)
...
void MainWindow::stop_audio(OATS::OutputPanel* op)
{
    if (op->schedule_item()->item_status() == OATS::ItemStatus::PLAYING)
    {
        ...
        op->schedule_item()->set_play_date(QDate::currentDate());
        op->schedule_item()->set_play_time(QTime::currentTime());
        ...
        QString sched_id = QString("Schedule ID: %1").arg(QString::number(op->schedule_item()->id()));
        log_info(sched_id);
        ...
        if (op->schedule_item()->schedule_type() == OATS::ScheduleType::COMM) {
           // mark_comm_as_played(op->schedule_item()->id());
            ...
            op->schedule_item()->set_db_update_status(OATS::DBUpdateStatus::Ready);
        }
    }
...
void MainWindow::pause_audio(OATS::OutputPanel* op)
{
    if (op->schedule_item()->item_status() == OATS::ItemStatus::PLAYING)
    {
        // TODO: Log this action

        op->set_panel_status(OATS::PanelStatus::PAUSED);
        op->schedule_item()->set_item_status(OATS::ItemStatus::PAUSED);
...
void MainWindow::pause_jingle(OATS::OutputPanel* op)
{
    if (op->schedule_item()->item_status() == OATS::ItemStatus::PLAYING)
    {
        // TODO: Log this action
...
void MainWindow::resume_audio(OATS::OutputPanel* op)
{
    op->set_panel_status(OATS::PanelStatus::PLAYING);
    op->schedule_item()->set_item_status(OATS::ItemStatus::PLAYING);
    op->schedule_item()->set_play_start_time(QTime::currentTime());
    ...
    QString msg = QString("Resume play: %1").arg(audio_file);
...
void MainWindow::resume_jingle(OATS::OutputPanel* op)
{
    op->set_panel_status(OATS::PanelStatus::PLAYING);
    op->schedule_item()->set_item_status(OATS::ItemStatus::PLAYING);
    op->schedule_item()->set_play_start_time(QTime::currentTime());
    ...
    //FIXME: full_audio_filename to read from cache
    QString audio_filename = audio->full_audio_filename();

    QString msg = QString("Resume play: %1").arg(audio_filename);
    log_info(msg);

    m_jingle_player->resume_audio();
    // display_schedule();
...
void MainWindow::mark_comm_as_played(int schedule_id, const QString base_filename )
{
    m_updatedb_mutex->try_lock();

    log_info("* mark comm break item *");

    for (auto& comm_break : m_comm_breaks[schedule_id]) {

        log_info("Comm: "+comm_break.base_filename);

        if (comm_break.base_filename == base_filename) {
            log_info("Mark: "+base_filename);
...
void MainWindow::update_item_status()
{
    for (const auto& item : m_schedule_items ) {
        ...
        {
            item->set_db_update_status(OATS::DBUpdateStatus::Done);
            create_schedule_item(item);
            continue;
        }

        if (item->db_update_status() == OATS::DBUpdateStatus::Ready) {
            update_item(item);
        }
    }
}
...
void MainWindow::create_schedule_item(const std::unique_ptr<OATS::ScheduleItem>& si)
{
    QString item_str = QString("Creating item...: %1").arg(si->audio()->title()->to_qstring());
    log_info(item_str);

    auto schedule = std::make_unique<Schedule>();

    schedule->set_audio(si->audio()->id());
    schedule->set_schedule_hour(si->hour());
    schedule->set_schedule_date(si->schedule_date());
    schedule->set_schedule_time(si->schedule_time());
    schedule->set_schedule_item_type(si->schedule_type_to_str().toStdString());
    schedule->set_auto_transition(static_cast<int>(si->transition_type()));
    schedule->set_play_date(si->play_date());
    schedule->set_play_time(si->play_time());
    schedule->set_play_status(si->item_status_text());
    schedule->set_comment(si->comment());

    auto sched_model = std::make_unique<EntityDataModel>(
        std::make_shared<Schedule>()
        );
    sched_model->createEntity(std::move(schedule));

}

void MainWindow::update_item(const std::unique_ptr<OATS::ScheduleItem>& si)
{
    Schedule schedule;

    schedule.setId(si->id());
    schedule.set_audio(si->audio()->id());
    schedule.set_schedule_date(si->schedule_date());
    schedule.set_schedule_hour(si->hour());
    schedule.set_schedule_time(si->schedule_time());
    ...
    sched_model->updateEntity(schedule);
}
...
void MainWindow::make_item_current(int schedule_ref, int grid_pos)
{
    ...
    int item_duration = si->audio()->playable_duration();
    ...
    int i = index_of(si->schedule_ref());
    schedule_item(i)->set_play_channel(ChannelA);

    display_schedule(0, i);
...
void MainWindow::play_cued_audio()
{
    for(auto const& op : m_output_panels) {
        if (op->panel_status() == OATS::PanelStatus::CUED) {
            play_button(op.get());
...
void MainWindow::stop_playing_audio()
{
    for(auto const& op : m_output_panels) {
        if (op->panel_status() == OATS::PanelStatus::PLAYING) {
            stop_button(op.get());
...
void MainWindow::fade_playing_audio()
{
    for(auto const& op : m_output_panels) {
        if (op->panel_status() == OATS::PanelStatus::PLAYING) {
            fade_button(op.get());
...
void MainWindow::pause_playing_audio()
{
    for(auto const& op: m_output_panels) {
        if (op->panel_status() == OATS::PanelStatus::PLAYING) {
            pause_button(op.get());
...
{
    //std::shared_ptr<AUDIO::Audio> audio = std::make_shared<AUDIO::Audio>(*m_track_browser->current_selected_audio());
    ...
    new_item->set_hour(new_time.hour());
...
void MainWindow::show_next_break_commercial()
{
    // Show next commercial break from the current item.
    ...
    for(int i=0; i<m_schedule_items.size()-1; ++i){
        ...
        if (m_schedule_items[i]->schedule_type() == OATS::ScheduleType::COMM)
        {
            ...
            if (schedule->break_time() >= current_time)
            {
                ...
                m_comm_viewer->set_title("Commercial Break: "+schedule->break_time().toString("HH:mm"));
                break;
            }
...
History MainWindow::make_history(int audio_id)
{
    ...
    for (auto& [name, entity] : edm->modelEntities()){
        ...
        history[sched->schedule_date()->value().dayOfWeek()].push_back(sched->schedule_hour()->value());
    }
...
{
    auto audio_duration = si->audio()->playable_duration();

    auto duration_seconds = (int)(audio_duration/1000);
...
void MainWindow::transition_stop(int item_ref, int grid_index)
{
    ...
    if (si->schedule_ref() == m_current_playing_item.item->schedule_ref())
        m_current_playing_item.item->set_transition_type(OATS::TransitionType::STOP);
...
void MainWindow::print_grid()
{
    ...
    for(const auto& item: m_schedule_grid){
        qDebug() << i++ << ":" << "Audio: " << item->schedule_item()->audio()->title()->to_qstring()
                 << "Play Channel: " << stoq(item->schedule_item()->play_channel());
    }
    qDebug() << " -------------------- ";
}
...
void MainWindow::fetch_commercial_from_db(int schedule_id)
{
    ...
    std::string where_filter = std::format(" where a.id = {}", std::to_string(schedule_id));
    std::string order_by = " Order by b.book_seq ";
...
void MainWindow::log_info(const QString info)
{
    Logger::info(class_name, info);

}

void MainWindow::log_error(const QString error)
{
    Logger::error(class_name, error);
