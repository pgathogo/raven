    ...
    int TreeViewModel::read_tree_data(const SelectedItems& items)
    {
        ...
        for (auto& row : rows){
            ...
            if (_prev_pid == row.parent_id){
                // Make leaf
                node = new TRAFFIK::Node(row.node_name, row.root_name, row.node_id, row.parent_id, row.node_id);
                node->setData(row.node_id, Qt::UserRole);
                m_nodes.push_back(node);
            } else{
                // Make header
                TRAFFIK::Node* node_root = new TRAFFIK::Node(row.root_name, row.node_name,  row.parent_id, -1, row.node_id);
                node_root->setData(row.node_id, Qt::UserRole);
                m_nodes.push_back(node_root);

                // ... then make a leaf
                TRAFFIK::Node* node_leaf = new TRAFFIK::Node(row.node_name, row.root_name, row.node_id, row.parent_id, row.node_id);
