    ...
    mOrder{order}
{
    ...
    connect(ui->cbBillingType, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &OrderForm::billingTypeCBChanged);

    connect(ui->cbBillingPeriod, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &OrderForm::billingPeriodCBChanged);

    connect(ui->cbBillingBasis, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &OrderForm::billingBasisCBChanged);


    connect(ui->cbAgencyCommType, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &OrderForm::agencyCommTypeCBChanged);

    connect(ui->cbSaleRepCommType, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &OrderForm::saleRepCommTypeCBChanged);

    connect(ui->cbTradeCreditType, QOverload<int>::of(&QComboBox::currentIndexChanged),
...
std::string OrderForm::windowTitle()
{
...
void OrderForm::setDefaults()
{
    ...
    setChoiceFieldDefault(ui->cbAgencyCommType, mSetup->agencyCommType()->value());

    ui->sbSaleRepComm->setValue(mSetup->saleRepComm()->value());
    setChoiceFieldDefault(ui->cbSaleRepCommType, mSetup->saleRepCommType()->value());
...
{
    if (i<0) return;
    EntityDataModel* edm = dynamic_cast<EntityDataModel*>(ui->cbAccountRep->model());
...
{
    if (i<0) return;
    EntityDataModel* edm = dynamic_cast<EntityDataModel*>(ui->cbPackage->model());
