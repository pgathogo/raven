...
namespace TRAFFIK{
    ...
    TraffikTreeViewModel::TraffikTreeViewModel(QList<QTableWidgetItem*> items, QObject *parent)
    ...
    int TraffikTreeViewModel::read_tree_data(const SelectedItems& items)
    {
        ...
        for (auto& row : rows){
            // create root node
            TRAFFIK::TraffikNode* node;
            if (_prev_pid == row.parent_id){
                // Make leaf
                node = new TRAFFIK::TraffikNode(row.node_name, row.root_name, row.node_id, row.parent_id, row.node_id);
                node->setData(row.node_id, Qt::UserRole);
                m_nodes.push_back(node);
            } else{
                // Make header
                TRAFFIK::TraffikNode* node_root = new TRAFFIK::TraffikNode(row.root_name, row.node_name,  row.parent_id, -1, row.node_id);
                node_root->setData(row.node_id, Qt::UserRole);
                m_nodes.push_back(node_root);

                // ... then make a leaf
                TRAFFIK::TraffikNode* node_leaf = new TRAFFIK::TraffikNode(row.node_name, row.root_name, row.node_id, row.parent_id, row.node_id);
    ...
    void TraffikTreeViewModel::build_tree(std::vector<TraffikNode *> &nodes)
    {
        for (auto node : nodes){
            if (node->parent_id() == -1){
    ...
    void TraffikTreeViewModel::insert_node(TreeNode tree_node, TraffikNode* node)
    {
        bool found=false;
    ...
    void TraffikTreeViewModel::print_tree(TreeNode tree_node, int level)
    {
        QString indent{""};
    ...
    TreeData TraffikTreeViewModel::tokenize(std::string line)
    {
         size_t pos = 0;
         std::string token;
